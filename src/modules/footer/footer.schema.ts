import Joi from "joi";
import * as globalField from "../../utils/globalFields";

export const addLinkSchema = {
  body: Joi.object({
    link: Joi.string().trim().lowercase().required().messages({
    ...
    arabic: Joi.object({
      title: Joi.string().min(2).trim().lowercase().required().messages({
...
export const updateLinkSchema = {
  body: Joi.object({
    link: Joi.string().trim().lowercase().messages({
      "string.base": "Link must be a string",
      "string.uri": "Link must be a valid URI",
    }),

    arabic: Joi.object({
      title: Joi.string().min(2).trim().lowercase().messages({
        "string.base": "Arabic title must be a string",
        "string.min": "Arabic title must be at least 2 characters",
      }),
    }),
...
export const addSocialSchema = {
  body: Joi.object({
    ...
    icon: Joi.string().trim().lowercase().required().messages({
...
export const updateSocialSchema = {
  body: Joi.object({
    link: Joi.string().uri().trim().lowercase().messages({
      "string.base": "Link must be a string",
      "string.uri": "Link must be a valid URL",
    }),

    icon: Joi.string().trim().lowercase().messages({
      "string.base": "Icon must be a string",
    }),

    display: Joi.boolean().messages({
