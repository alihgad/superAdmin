...
import SliderModel from "../../db/models/slider";
    ...
    if (section.images?.length > 0) {
      for (const image of section.images) {
        await cloudinary.uploader
          ...
          .catch((err) =>
            next(new Error("Image deletion failed " + err.message + ""))
          );
      }
    }
    section.images = [];
...
export const addSection = async (req, res, next) => {
  ...
  let sectionExists = await sectionModel.findOne({
    section: req.params.section,
    title: english.title,
    page: req.params.page,
  });
...
export const deleteSection = async (req, res, next) => {
  let section = await sectionModel.findOneAndDelete({
    section: req.params.section,
  });
  if (!section) {
    return next(new Error("wrong section name"));
...
export let deleteSlide = async (req, res, next) => {
  ...
  slider.slides = slider.slides.filter(
    (slide) => slide._id != req.params.slideId
  );
  await slider.save();
  return res
    .status(200)
